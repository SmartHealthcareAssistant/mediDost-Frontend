...
export default function ChatBot() {
  ...
  const fetchNewSession = async () => {
    try {
      const response = await axios.get("https://smart-healthcare-app-e0cx.onrender.com/api/session");
  ...
  const handleSend = async (text = input) => {
    ...
    try {
      // 2. Send to Backend
      const response = await axios.post("https://smart-healthcare-app-e0cx.onrender.com/api/chat", {
        message: text,
        sessionId,
      });
