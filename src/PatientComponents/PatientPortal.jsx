...
const [selectedSlot, setSelectedSlot] = useState(null);
  ...
  useEffect(() => {
  const s = io("https://smart-healthcare-app-e0cx.onrender.com");
  setSocket(s);
...
{prescriptions.map((p) => (
    ...
    {p.files?.length > 0 && (
      <div className="mt-2">
        {p.files.map((f, i) => (
          <a
            key={i}
            href={`https://smart-healthcare-app-e0cx.onrender.com${f}`}
            target="_blank"
            rel="noreferrer"
  ...
  src={
    d.image
      ? `https://smart-healthcare-app-e0cx.onrender.com${d.image}`
